...
WORKDIR /src
COPY ["src/IkerFinance.API/IkerFinance.API.csproj", "src/IkerFinance.API/"]
...
COPY . .
WORKDIR "/src/src/IkerFinance.API"
RUN dotnet build "IkerFinance.API.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "IkerFinance.API.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "IkerFinance.API.dll"]
